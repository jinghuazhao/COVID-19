...
R --no-save -q <<END
  ...
  ct <- snpstats[ord,]
  all <- within(ct,{chr <- chromosome; pos <- position; snp <- rsid; a1 <- A; a2 <- B; qual <- "."; filter <- ".";})
...
# Or the INFO fields you want in the ClinVar VCF file
  ...
  vep --i ukb-ACE2.vcf --species homo_sapiens -o ukb-ACE2.vepout --cache --offline --force_overwrite \
